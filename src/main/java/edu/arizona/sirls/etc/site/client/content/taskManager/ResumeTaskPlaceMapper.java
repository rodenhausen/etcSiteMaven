...
public class ResumeTaskPlaceMapper {
	...
	public Place getPlace(Task task) {
		...
		case SEMANTIC_MARKUP:
			switch(edu.arizona.sirls.etc.site.shared.rpc.semanticMarkup.TaskStageEnum.valueOf(task.getTaskStage().getTaskStage())) {
				case INPUT:
		...
		case MATRIX_GENERATION:
			switch(edu.arizona.sirls.etc.site.shared.rpc.matrixGeneration.TaskStageEnum.valueOf(task.getTaskStage().getTaskStage())) {
				case INPUT:
			...
			return new edu.arizona.sirls.etc.site.client.content.matrixGeneration.MatrixGenerationInputPlace();
